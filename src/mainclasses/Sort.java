...
public class Sort {
    ...
    // Sort by Player Health from the biggest to the smallest with Selection Sort
    // (Method Overloading)
    // Nama : Ricky
    // NIM : 03082200012
    ...
    /*
     * Nama : Richard Hanitio NIM : 03082200014
     */
    ...
    /*
     * Nama : Calvin Wendy NIM : 03082200010
     */
    ...
    // Sort Enemy Health from the lowest to the highest amount using insertion sort
    // Nama : Setiawan Junior
    // NIM : 03082180037
    public Enemy[] quickSort(Enemy[] enemy) {
        Enemy sortedEnemyList[] = enemy;
        for (int i = 1; i < sortedEnemyList.length; i++) {
            int j = i;
            while ((j > 0) && (sortedEnemyList[j - 1].getHealth() > sortedEnemyList[j].getHealth())) {
                Enemy tampungan = sortedEnemyList[j];
                sortedEnemyList[j] = sortedEnemyList[j - 1];
                sortedEnemyList[j - 1] = tampungan;
                j = j - 1;
            }
        }
        return sortedEnemyList;
    ...
    // Sort Character class strength (STR) from the lowest to the highest amount
    // using insertion sort
    // Nama : Setiawan Junior
    // NIM : 03082180037
    public Player[] quickSort(Player[] player) {
        Player sortedPlayerList[] = player;
        for (int i = 1; i < sortedPlayerList.length; i++) {
            int j = i;
            while ((j > 0)
                    && (sortedPlayerList[j - 1].getClassOriginalSTR() > sortedPlayerList[j].getClassOriginalSTR())) {
                Player tampungan = sortedPlayerList[j];
                sortedPlayerList[j] = sortedPlayerList[j - 1];
                sortedPlayerList[j - 1] = tampungan;
                j = j - 1;
            }
        }
        return sortedPlayerList;
